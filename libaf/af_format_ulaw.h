    ...
    if (af_format_is_integer(format)) {
        ...
        case (1):
            for (i = 0; i < len; i++) {
                if (((int8_t *)in)[i] & 0x80)
                    ((int8_t *)out)[i] =
                        (-1 * ulaw_decode[(((int8_t *)in)[i]) & 0x7F]) >> 8;
                else
                    ((int8_t *)out)[i] =
                        (ulaw_decode[(((int8_t *)in)[i]) & 0x7F]) >> 8;
            }
            break;
        case (2):
            for (i = 0; i < len; i++) {
                if (((int8_t *)in)[i] & 0x80)
                    ((int16_t *)out)[i] = -1 *
                                          ulaw_decode[(((int8_t *)in)[i]) &
                                                      0x7F];
                else
                    ((int16_t *)out)[i] =
                        ulaw_decode[(((int8_t *)in)[i]) & 0x7F];
            }
            break;
        case (4):
            for (i = 0; i < len; i++) {
                if (((int8_t *)in)[i] & 0x80)
                    ((int32_t *)out)[i] =
                        (-1 * ulaw_decode[(((int8_t *)in)[i]) & 0x7F]) << 16;
                else
                    ((int32_t *)out)[i] =
                        (ulaw_decode[(((int8_t *)in)[i]) & 0x7F]) << 16;
            }
            break;
        default:
            return AF_ERROR;
    ...
    if (af_format_is_integer(format)) {
        ...
        case (4):
            for (i = 0; i < len; i++) {
                if (((int32_t *)in)[i] >= 0)
                    ((int8_t *)out)[i] =
                        ulaw_encode[((int32_t *)in)[i] >> (16 + 2)];
                else
                    ((int8_t *)out)[i] = 0x7F &
                                         ulaw_encode[-((int32_t *)in)[i] >>
                                                     (16 + 2)];
            }
            break;
        default:
        ...
        for (i = 0; i < len; i++) {
            if (((float *)in)[i] >= 0)
                ((int8_t *)out)[i] =
                    ulaw_encode[(int)(32767.0 / 4.0 * ((float *)in)[i])];
            else
                ((int8_t *)out)[i] = 0x7F &
                                     ulaw_encode[(int)(-32767.0 / 4.0 *
                                                       ((float *)in)[i])];
        }
    }
    return AF_OK;
