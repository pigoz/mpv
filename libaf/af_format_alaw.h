    ...
    if ((format & AF_FORMAT_POINT_MASK) == AF_FORMAT_I) {
        ...
        case (1):
            for (i = 0; i < len; i++) {
                if (((int8_t *)in)[i] & 0x80)
                    ((int8_t *)out)[i] =
                        (-1 * alaw_decode[(((int8_t *)in)[i]) & 0x7F]) >> 8;
                else
                    ((int8_t *)out)[i] =
                        (alaw_decode[(((int8_t *)in)[i]) & 0x7F]) >> 8;
            }
            break;
        case (2):
            for (i = 0; i < len; i++) {
                if (((int8_t *)in)[i] & 0x80)
                    ((int16_t *)out)[i] = -1 *
                                          alaw_decode[(((int8_t *)in)[i]) &
                                                      0x7F];
                else
                    ((int16_t *)out)[i] =
                        alaw_decode[(((int8_t *)in)[i]) & 0x7F];
            }
            break;
        case (4):
            for (i = 0; i < len; i++) {
                if (((int8_t *)in)[i] & 0x80)
                    ((int32_t *)out)[i] =
                        (-1 * alaw_decode[(((int8_t *)in)[i]) & 0x7F]) << 16;
                else
                    ((int32_t *)out)[i] =
                        (alaw_decode[(((int8_t *)in)[i]) & 0x7F]) << 16;
            }
            break;
        default:
            return AF_ERROR;
        ...
        for (i = 0; i < len; i++) {
            if (((int8_t *)in)[i] & 0x80)
                ((float *)out)[i] = -1.0 / 32768.0 *
                                    (float)alaw_decode[(((int8_t *)in)[i]) &
                                                       0x7F];
            else
                ((float *)out)[i] = +1.0 / 32768.0 *
                                    (float)alaw_decode[(((int8_t *)in)[i]) &
    ...
    if ((format & AF_FORMAT_POINT_MASK) == AF_FORMAT_I) {
        ...
        case (4):
            for (i = 0; i < len; i++) {
                if (((int32_t *)in)[i] >= 0)
                    ((int8_t *)out)[i] =
                        alaw_encode[((int32_t *)in)[i] >> (16 + 4)];
                else
                    ((int8_t *)out)[i] = 0x7F &
                                         alaw_encode[-((int32_t *)in)[i] >>
                                                     (16 + 4)];
            }
            break;
        default:
        ...
        for (i = 0; i < len; i++) {
            if (((float *)in)[i] >= 0)
                ((int8_t *)out)[i] =
                    alaw_encode[(int)(32767.0 / 16.0 * ((float *)in)[i])];
            else
                ((int8_t *)out)[i] = 0x7F &
                                     alaw_encode[(int)(-32767.0 / 16.0 *
                                                       ((float *)in)[i])];
        }
    }
    return AF_OK;
