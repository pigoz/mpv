...
static void print_fmt(struct af_data_s *d)
{
    if (d) {
...
{
  do{
    struct af_data_s in; // Format of the input to current filter
    ...
    if(!af->prev)
      memcpy(&in,&(s->input),sizeof(struct af_data_s));
    else
      memcpy(&in,af->prev->data,sizeof(struct af_data_s));
    // Reset just in case...
    in.audio=NULL;
    in.len=0;
	...
	if((af->prev?af->prev->data->nch:s->input.nch) != in.nch){
	  ...
	  if(!new->prev)
	    ...
	    memcpy(&in,new->prev->data,sizeof(struct af_data_s));
...
int af_lencalc(double mul, struct af_data_s* d)
{
  int t = d->bps * d->nch;
