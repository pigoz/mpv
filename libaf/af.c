...
static af_instance_t *af_prepend(af_stream_t *s, af_instance_t *af,
                                 const char *name)
{
...
/* Create and insert a new filter of type name after the filter in the
   argument. This function can be called during runtime, the return
   value is the new filter */
static af_instance_t *af_append(af_stream_t *s, af_instance_t *af,
                                const char *name)
{
    // Create the new filter and make sure it is OK
    af_instance_t *new = af_create(s, name);
    ...
    do {
        ...
        case AF_FALSE: { // Configuration filter is needed
            ...
            if ((AF_INIT_TYPE_MASK & s->cfg.force) != AF_INIT_FORCE) {
                ...
                if ((af->prev ? af->prev->data->nch : s->input.nch) !=
                    in.nch) {
...
int af_init(af_stream_t *s)
{
    ...
    if ((AF_INIT_TYPE_MASK & s->cfg.force) != AF_INIT_FORCE) {
        ...
        if (s->output.rate && s->last->data->rate != s->output.rate) {
            ...
            af =
                af_control_any_rev(s, AF_CONTROL_RESAMPLE_RATE | AF_CONTROL_SET,
                                   &(s->output.rate));
            if (!af) {
                ...
                if (!af ||
                    (AF_OK !=
                     af->control(af, AF_CONTROL_RESAMPLE_RATE | AF_CONTROL_SET,
                                 &(s->output.rate))))
                    return -1;
