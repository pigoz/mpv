...
static int control(struct af_instance_s *af, int cmd, void *arg)
{
    ...
    mp_msg(MSGT_AFILTER, MSGL_ERR, "[lavr] control ok.\n");
    return AF_UNKNOWN;
}

static void uninit(struct af_instance_s *af)
{
    mp_msg(MSGT_AFILTER, MSGL_ERR, "[lavr] uinit ok.\n");
    talloc_free(af->data);
}

static af_data_t* play(struct af_instance_s *af, struct af_data_s *data)
{
    mp_msg(MSGT_AFILTER, MSGL_ERR, "[lavr] play ok.\n");
    return data;
}
...
static int af_open(struct af_instance_s *af){
    ...
    if (af->data == NULL) {
        mp_msg(MSGT_AFILTER, MSGL_ERR, "[lavr] init ok (loal).\n");
        return AF_ERROR;
    }

    mp_msg(MSGT_AFILTER, MSGL_ERR, "[lavr] init ok.\n");
    return AF_OK;
}

af_info_t af_info_lavr = {
    "Audio resampling filter based on libavresample",
    "lavr",
    "Stefano Pigozzi",
    "",
    AF_FLAGS_REENTRANT,
