...
static double written_audio_pts(struct MPContext *mpctx)
{
    ...
    if (a_pts != MP_NOPTS_VALUE)
        ...
        if (sh_audio->i_bps)
            a_pts += (ds_tell_pts(d_audio) - sh_audio->a_in_buffer->free_offset) /
    ...
    a_pts -= sh_audio->a_buffer->free_offset / (double)sh_audio->o_bps;
...
static void seek_reset(struct MPContext *mpctx, bool reset_ao, bool reset_ac)
{
    ...
    if (mpctx->sh_audio && reset_ac) {
        ...
        mpctx->sh_audio->a_buffer->free_offset = 0;
    }
