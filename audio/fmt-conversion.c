...
static const struct {
    ...
    {AV_SAMPLE_FMT_S16P,  AF_FORMAT_S16P_NE},
    {AV_SAMPLE_FMT_S32P,  AF_FORMAT_S32P_NE},
    {AV_SAMPLE_FMT_FLTP,  AF_FORMAT_FLOATP_NE},
...
enum AVSampleFormat afsamplefmt2avsamplefmt(int fmt)
{
    ...
    if (sample_fmt == AF_FORMAT_UNKNOWN)
        mp_msg(MSGT_GLOBAL, MSGL_ERR, "Unsupported sample format: %s\n",
...
{
    int i;
    for (i = 0; audio_conversion_map[i].sample_fmt != AF_FORMAT_UNKNOWN; i++)
