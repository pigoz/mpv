...
static int init_audio_codec(sh_audio_t *sh_audio)
{
    ...
    if (sh_audio->audio_in_minsize > 0) {
        ...
        sh_audio->a_in_buffer = mp_audio_new(NULL, sh_audio->audio_in_minsize);
    }
    ...
    sh_audio->a_buffer =
        mp_audio_new(NULL, base_size + sh_audio->audio_out_minsize);
...
void uninit_audio(sh_audio_t *sh_audio)
{
    ...
    mp_audio_free(sh_audio->a_buffer);
    mp_audio_free(sh_audio->a_in_buffer);
}
                       ...
                       int *out_samplerate, int *out_channels, int *out_format)
{
    ...
    mp_tmsg(MSGT_DECAUDIO, MSGL_V,
            ...
            af_fmt2str_short(
                afs->input.format), afs->output.rate,
